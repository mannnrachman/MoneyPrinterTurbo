...
将一段长视频（上传或 YouTube 链接）自动切分成多个竖屏 9:16 短视频。适合：播客、访谈、教程、讲座、单人口播、音乐/歌曲等。
...
视频源（上传 / YouTube URL）
  ...
  → Whisper 转写（含时间戳）
  → 生成候选片段（按句子边界聚合）
  → [可选] LLM 打分选出高光片段 / 或均匀取片
  → FFmpeg 渲染竖屏 9:16（背景模糊 + 前景居中）
```

### WebUI 功能­矩

在 WebUI 的「Long Video to Shorts」面板中：

- **上传长视频**：支持 MP4、MOV、AVI、FLV、MKV、WEBM（最大 200MB）。
- **YouTube URL**：粘贴链接直接下载并剪辑；支持选择最大分辨率（Auto / 360p / 480p / 720p / 1080p / 1440p / 4K）。
- **Fetch Video Info**：先获取视频标题、时长和实际可用的分辨率，再选择下载质量。
- **预览 / 编辑片段 (Preview Clips)**：先点击「Preview Clips」转写并生成候选片段。上方是较小的实际视频预览，下方是带时间尺和音频波形的编辑时间线：
  - 时间线的 `− / + / Fit` 只缩放**波形时间线**，不会放大视频；放大后可横向滚动并精确到 0.1 秒，缩小时可看到更长的全局范围；
  - 每个红色片段都有左右拖动手柄，拖动左手柄设置开始时间，拖动右手柄设置结束时间；蓝色播放头随视频播放移动，点击波形可跳转播放；
  - 点击「Delete」**删除**不想要的片段；
  - 点击「Add Segment」**新增**片段（默认取视频末尾 15 秒）；
  - 编辑满意后再点「Generate Clips」，程序**只渲染你选定的片段**（跳过重新转写与 LLM 打分）。
- **Clip Count（短视频数量）**：1–20，目标剪出多少个短视频。
- **Clip Duration（短视频时长·秒）**：10–60 秒，目标时长区间。源视频较短时自动生成重叠片段以满足数量。
- **Clip Focus Prompt（重点提示）**：可选，告诉 LLM 更偏好哪类内容（如“最搞笑的片段”“核心商业建议”）。
- **Whisper Model（Whisper 模型）**：`tiny / base / small / medium / large-v3`。越大越准越慢；首次使用自动下载。
- **VAD Filter（VAD 过滤器）**：
  - **开启**：跳过静音/纯音乐区间，转写更快，适合演讲、播客。
  - **关闭**：转写全部声音，保留歌声/音乐内容（默认关闭）。
- **Generate Clips（生成）**：后台任务，可看到进度与日志；完成后每个片段有**预览与下载按钮**。

完成后的源视频与中间音频会自动删除，不占磁盘。

### 处理详情

- 片段边界**对齐句子**，不会在句子中间切断。
- 无 LLM API Key 时自动降级为**均匀取片**，仍可产出短视频。
- 输出固定 **1080×1920 (9:16)**，H.264 + AAC。

### 相关配置

```toml
[whisper]
# 片段的 VAD 开关（true=跳过静音/音乐，false=全部转写）
use_vad = false
# 片段使用的 Whisper 模型
model_size = "small"
```
...
- **我只得到一个片段？** 源视频剩余部分可能是静音/纯音乐且被 VAD 跳过，关闭 VAD 或改用「Preview Clips」检查片段。
- **YouTube 下载报 HTTP 403？** YouTube 会拦截数据中心/异常 IP。已内置 PO Token (bgutil) + nightly yt-dlp 方案；仍失败时尝试调低分辨率或稍后重试。
- **如何只要指定区间？** 先「Preview Clips」，把其他片段删掉，只保留想要的区间再生成。
